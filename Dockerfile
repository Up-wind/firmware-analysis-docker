...
RUN unzip /tmp/files/FirmAFL.zip -d /root/ && \
...
RUN unzip /tmp/files/console.zip -d /root/FirmAFL/firmadyne/sources/ && \
    ...
    unzip /tmp/files/extractor.zip -d /root/FirmAFL/firmadyne/sources/ && \
    ...
    unzip /tmp/files/libnvram.zip -d /root/FirmAFL/firmadyne/sources/ && \
    ...
    unzip /tmp/files/scraper.zip -d /root/FirmAFL/firmadyne/sources/ && \
...
RUN unzip /tmp/binwalk-2.zip -d /root/FirmAFL/ && \
    ...
    unzip /tmp/binwalk-3.zip -d /root/ && \
...
RUN cp /tmp/files/data.xz /root/FirmAFL/firmadyne/database/ && \
...
RUN sudo -u postgres mkdir -p /data/postgresql/data && \
